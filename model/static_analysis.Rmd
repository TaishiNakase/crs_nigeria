...
Let's compute the 30-year CRS burden for each state under increasing levels of routine rubella vaccination coverage. We also compute the CRS burden in the last 30-th year of each simtulation. 
...
Let's also show the minimum routine vaccination coverage required to eliminate CRS cases after 30 years of vaccination. 

```{r warning=FALSE}
plot_min_elim_vac_cov <- function(crs_burden_dist, min_vac_cov, ng1) {
  summarised_crs_burden_dist <- crs_burden_dist %>%
    filter(smoothed_crs_burden_lastyear<1) %>% 
    mutate(vac=as.numeric(as.character(vac))) %>% 
    group_by(state) %>% 
    filter(vac==min(vac)) %>%
    right_join(unique(select(crs_burden_dist, state)), by="state") %>% 
    left_join(filter(min_vac_cov, metric=="median"), by="state") %>% 
    left_join(ng1, by="state") %>%
    mutate(zone=ifelse(state=="Nigeria", "National", zone))
  ...
  min_elim_vac_cov_map <- ggplot() + 
    geom_sf(data=filter(summarised_crs_burden_dist, state!="Nigeria"), 
            aes(geometry=geometry, fill=vac)) + 
    scale_fill_gradientn(breaks=seq(0, 100, 20), limits=c(0, 100), colors=rep(pal, each=10), 
                         labels=paste(seq(0, 100, 20), "%", sep=""), name="Necessary\ncoverage", 
                         guide=guide_colorbar(frame.colour="black", ticks.colour="black", title.position="top")) + 
    theme_bw() + 
    theme(legend.position=c(0.85, 0.2), legend.key.size=unit(0.5, "cm"), legend.key.width=unit(0.6, "cm"), 
          legend.key.height=unit(0.4, "cm"), 
          legend.text=element_text(size=8), legend.title=element_text(size=8), 
          legend.box.background=element_blank(), legend.background=element_blank()) + 
    theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
  ...
  zone_colors <- c(brewer.pal(9, "Set1")[c(1:5, 7)], "grey")
  zone_order <- c("North Central", "North East", "North West", 
                  "South East", "South South", "South West", "National")
  min_vac_cov_comparison <- ggplot(data=summarised_crs_burden_dist) + 
    geom_point(mapping=aes(x=min_cov*100, y=vac, color=zone)) + 
    scale_x_continuous(limits=c(0, 100), breaks=seq(0, 100, 25), labels=paste0(seq(0, 100, 25), "%")) + 
    scale_y_continuous(limits=c(0, 100), breaks=seq(0, 100, 25), labels=paste0(seq(0, 100, 25), "%")) + 
    scale_color_manual(breaks=zone_order, values=zone_colors) + 
    labs(x="Routine coverage necessary for safe introduction (%)", 
         y="Routine coverage necessary for elimination (%)") + 
    theme_bw() + 
    theme(legend.position = c(0.8, 0.5), legend.key.size=unit(0.3, "cm"), 
  ...
  pAB <- cowplot::plot_grid(min_elim_vac_cov_map, min_vac_cov_comparison, nrow=1, 
                            labels=c("(a)", "(b)"), label_size=10, label_fontface="plain", label_fontfamily="serif")
  return(pAB)
}
min_elim_vac_cov <- plot_min_elim_vac_cov(crs_burden_dist=state_landscape_crs_burden_dist, 
                                          min_vac_cov=min_vac_cov_data, 
                                          ng1=ng1)
scale_factor <- 0.3
pdf(file=file.path(FIGURES_FOLDER, "min_elim_vac_cov.pdf"), w=8.5, h=11*scale_factor)
print(min_elim_vac_cov)
a <- dev.off()
```
