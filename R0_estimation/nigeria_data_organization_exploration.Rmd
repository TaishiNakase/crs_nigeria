...
+ Imego-Afon, Ogun does not have population data due to inconsistencies between the GADM polygons and the WorldPop grids, so it is excluded from the analyses. 
...
Downloading the polygons (if necessary) from [GADM](https://gadm.org). Data can also be manually downloaded from this [link](https://hub.worldpop.org/geodata/summary?id=16883). 
...
```{r}
ng0 <- readRDS(file.path(SPATIAL_POLYGONS_DATA_FOLDER, "gadm36_NGA_0_sf.rds")) %>% # country polygon
  transmute(country = str_squish(NAME_0))

ng1 <- readRDS(file.path(SPATIAL_POLYGONS_DATA_FOLDER, "gadm36_NGA_1_sf.rds")) %>% # state polygons
  transmute(state = str_squish(NAME_1))

ng2 <- readRDS(file.path(SPATIAL_POLYGONS_DATA_FOLDER, "gadm36_NGA_2_sf.rds")) %>% # LGA polygons
  transmute(state = str_squish(NAME_1), 
...
```{r}
simplified_ng0 <- st_simplify(st_make_valid(ng0), dTolerance=1000)
simplified_ng1 <- st_simplify(st_make_valid(ng1), dTolerance=1000)
simplified_ng2 <- st_simplify(st_make_valid(ng2), dTolerance=1000)
```
...
Downloading the population data from [WorldPop](https://www.worldpop.org):

```{r eval=FALSE}
get_worldpop_data <- function(gender, agegrp) {
  link <- paste("https://data.worldpop.org/GIS/AgeSex_structures/Global_2000_2020/2020/NGA/nga", 
                gender, agegrp, "2020.tif", sep="_")
...
input <- expand.grid(gender=genders, agegrp=agegrps) %>% head(1)
...
```{r eval=FALSE}
lga_population <- function(agegrp_gender_mat, this_lga, this_state, nga_pop, ng=ng2) {
  ...
  subset_population <- function(this_agegrp, this_gender) {
    n <- lga_pop[, grepl(paste(this_gender, this_agegrp, "", sep="_"), names(lga_pop), fixed=TRUE)] %>%
      sum(na.rm=TRUE)
    return(n)
  }
  df <- apply(agegrp_gender_mat, 1, function(x) subset_population(x["agegrp"], x["gender"])) %>%
    cbind(agegrp_gender_mat, n=.) %>%
    cbind(LGA=rep(this_lga, nrow(.)), .) %>%
    cbind(state=rep(this_state, nrow(.)), .)
  return(df)
}
all_files <- list.files(path=POPULATION_DATA_FOLDER, pattern = "2020.tif")
ngapop_stars_proxy <- do.call(c, purrr::map(all_files, read_stars))
agegrp_gender_mat <- expand.grid(gender=c("f", "m"), agegrp=c("0", "1", as.character(seq(5, 80, 5))))
regions <- select(ng2, state, LGA) %>% st_drop_geometry() %>% unique()
regions <- regions[regions$LGA != "Imeko-Afon", ] # remove Imeko-Afon
nga_pop <- apply(regions, 1, function(x) lga_population(agegrp_gender_mat=agegrp_gender_mat, 
                                                        this_lga=x["LGA"], this_state=x["state"],
                                                        nga_pop=ngapop_stars_proxy)) %>%
  do.call(rbind, .)
rownames(nga_pop) <- NULL
imeko_agon_ogun <- data.frame(state="Ogun", LGA="Imeko-Afon", gender=c("f", "m"), 
                              agegrp=rep(as.character(c(0, 1, seq(5, 80, 5))), each=2), n=rep(NA, 36))
nga_pop %<>% rbind(., imeko_agon_ogun)
